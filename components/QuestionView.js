...
import MCQField from "./input_components/MCQField";
import InputField from "./input_components/InputField";
import RadioInputField from "./input_components/RadioField";

const QuestionView = ({ title, questions, computeResult }) => {
  ...
  const saveAnswer = (question_id, selectedAnswer) => {
    answers[question_id] = selectedAnswer;
    setAnswers(answers);
    // console.log("answer", answers);
  };

  const renderOptions = (question_id, type, options) => {
    if (type === "MCQ") {
      return (
        <MCQField
          inputValues={options}
      ...
      return (
        <InputField saveAnswer={(answer) => saveAnswer(question_id, answer)} />
      );
